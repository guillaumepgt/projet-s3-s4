...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($nom) && !empty($prix) && !empty($code_barre)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO produits (nom, prix, code_barre) 
                 VALUES (:nom, :prix, :code_barre)'
            );
            ...
            $stmt->execute([
                ...
                ':code_barre' => htmlspecialchars($code_barre),
            ]);

            // Message de succès
            echo "<p>Le produit <strong>" . htmlspecialchars($nom) . "</strong> a été ajouté avec succès !</p>";
